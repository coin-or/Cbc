...
// Guts of copy
void 
CbcHeuristicDW::gutsOfCopy(const CbcHeuristicDW & rhs)
{
...
void
CbcHeuristicDW::generateCpp( FILE * fp)
{
  abort();
}
...
void
CbcHeuristicDW::resetModel(CbcModel * model)
{
  if (model_&&numberBlocks_&&
      model->getNumCols()!=model->getNumCols())
    abort();
  model_=model;
}
/*
...
int
  CbcHeuristicDW::solution(double & solutionValue,
...
#define SCALE_FACTOR 1.0
	...
	for (int iBlock=0;iBlock<numberBlocks_;iBlock++) {
	  ...
	  ClpSimplex * tempModel = new
	    ClpSimplex(solver->getModelPtr(),
	...
	if (continuousSolution_) {
	  blockDiff[iBlock] += 
	    fabs((bestSolution_[iColumn]-continuousSolution_[iColumn]))
...
#define TRY_ADJUST 3
	...
	if (model.bestSolution() && 
	    model.getMinimizationObjValue()<lastObjective_) {
	  const int * original = pinfo.originalColumns();
...
// update model
void 
CbcHeuristicDW::setModel(CbcModel * model)
{
  if (model!=model_) {
    gutsOfDelete();
    model_ = model;
...
// Pass in a solution
void 
CbcHeuristicDW::passInSolution(const double * solution)
{
  if (fullDWEverySoOften_>0) {
...
void 
CbcHeuristicDW::passInContinuousSolution(const double * solution)
{
  int numberColumns = solver_->getNumCols();
  if (!continuousSolution_) 
...
int 
CbcHeuristicDW::numberColumnsDW(int whichDW) const
{
  if (whichDW>=numberDWTimes_)
    return COIN_INT_MAX;
  else
