...
This version shows use of Osi objects rather than Cbc objects
************************************************************************/
...
int main (int argc, const char *argv[])
{
  ...
  */
  ...
  /* This defines branching stuff.
     The user can derive from OsiChoose to get required functionality
  */
  CbcBranchDefaultDecision decision;
  // Need to find integers in Osi land
  model.solver()->findIntegersAndSOS(false);
  OsiChooseStrong choose(model.solver());
  // Do more strong branching if small
  if (model.getNumCols()<5000)
    choose.setNumberStrong(10);
  else
    choose.setNumberStrong(5);
  choose.setNumberBeforeTrusted(5);
