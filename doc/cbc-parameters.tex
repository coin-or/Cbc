Parsing --help output...
Found 15 topics, 221 parameters
  querying parameter 20/221...
  querying parameter 40/221...
  querying parameter 60/221...
  querying parameter 80/221...
  querying parameter 100/221...
  querying parameter 120/221...
  querying parameter 140/221...
  querying parameter 160/221...
  querying parameter 180/221...
  querying parameter 200/221...
Queried 216 unique parameters
...
For some problems, cut generators and general branching work better if the problem would be infeasible if the cost is too high. If this option is enabled, the objective function is added as a constraint which right hand side is set to the current cutoff value (objective value of best known solution) <current: off>

\textbf{Values:} \texttt{off}, \texttt{on}, \texttt{variable}, \texttt{forcevariable}, \texttt{conflict}
...
If the gap between best solution and best possible solution is less than this then the search will be terminated. Also see ratioGap. current 1e-12>

\textbf{Range:} 0 to 1e+20
...
All solutions must be better than this value (in a minimization sense). This is also set by cbc whenever it obtains a solution and is set to the value of the objective for the solution minus the cutoff increment. current 1e+50>

\textbf{Range:} -1e+60 to 1e+60
...
If the gap between the best solution and the best possible solution is less than this fraction of the objective value at the root node then the search will terminate.  See 'allowableGap' for a way of using absolute value rather than fraction. current 0>
...
This is a repeatable way to limit search.  Normally using time is easier but then the results may not be repeatable. current 2147483647>

\textbf{Range:} 1 to 2147483647
...
This criterion specifies that when a feasible solution is available, the search should continue only if better feasible solutions were produced in the last nodes. current 2147483647>
...
You may want to stop after (say) two solutions or an hour. This is checked every node in tree, so it is possible to get more solutions from heuristics. current 1073741823>

\textbf{Range:} 1 to 1073741823
...
After this many seconds the program will act as if maximum nodes had been reached. You may wish to also set '-check less' which stops cbc checking time quite as often which reduces system time. current 1e+08>

\textbf{Range:} -1 to 1e+12
...
After this many seconds clp will act as if maximum iterations had been reached (if value >=0). current -1>

\textbf{Range:} -1 to 1.79769e+308
...
This switches on clique cuts (either at root or in entire tree). An improved version of the Bron-Kerbosch algorithm is used to separate cliques. onglobal <current: ifmove>

\textbf{Values:} \texttt{off}, \texttt{on}, \texttt{root}, \texttt{ifmove}, \texttt{forceon}
...
This can be used to switch on or off all cuts (apart from Reduce and Split).  Then you can set individual ones off or on.  See branchAndCut for information on options. <current: on>
...
The original cuts - beware of imitations!  Having gone out of favor, they are now more fashionable as LP solvers are more robust and they interact well with other cuts.  They will almost always give cuts (although in this executable they are limited as to number of variables in cut).  However the cuts may be dense so it is worth experimenting (Long allows any length). Value 'on' enables the cut generator and fine tune the behavior). Value 'root' lets CBC run the cut generator generate only at the root node. Value 'ifmove' lets CBC use the cut generator in the tree if it looks as if it is doing some good and moves the objective value. Value 'forceon' turns on the cut generator and forces CBC to use it at every node. Reference: https://github.com/coin-or/Cgl/wiki/CglGomory <current: ifmove>

\textbf{Values:} \texttt{forceandglobal}, \texttt{forcelongon}, \texttt{onglobal}, \texttt{longer}, \texttt{shorter}
...
\textbf{Values:} \texttt{forceandglobal}, \texttt{onglobal}
...
This is a Lagrangean relaxation for Twomir cuts.  See lagomoryCuts for description of options. <current: off>

\textbf{Values:} \texttt{cleaninstead}, \texttt{bothinstead}
...
Value 'on' enables the cut generator and CBC will try it in the branch and cut tree (see cutDepth on how to fine tune the behavior). Value 'root' lets CBC run the cut generator generate only at the root node. Value 'ifmove' lets CBC use the cut generator in the tree if it looks as if it is doing some good and moves the objective value. Value 'forceon' turns on the cut generator and forces CBC to use it at every node. Reference: https://github.com/coin-or/Cgl/wiki/CglMixedIntegerRounding2 onglobal <current: ifmove>
...
This switches on odd-wheel inequalities (either at root or in entire tree). onglobal <current: off>
...
Value 'forceOnBut' turns on probing and forces CBC to do probing at every node, but does only probing, not strengthening etc. Value 'strong' forces CBC to strongly do probing at every node, that is, also when 'forceonbutstrong' is like 'forceonstrong', but does only probing (column fixing) and turns off row strengthening, so the matrix will not change inside the branch and bound.Reference: https://github.com/coin-or/Cgl/wiki/CglProbing strongroot <current: ifmove>

\textbf{Values:} \texttt{forceonbut}, \texttt{forceonbutstrong}, \texttt{forceonglobal}, \texttt{forceonstrong}, \texttt{onglobal}
...
Value 'on' enables the cut generator and CBC will try it in the branch and cut tree (see cutDepth on how to fine tune the behavior). Value 'root' lets CBC run the cut generator generate only at the root node. Value 'ifmove' lets CBC use the cut generator in the tree if it looks as if it is doing some good and moves the objective value. Value 'forceon' turns on the cut generator and forces CBC to use it at every node. Reference: https://github.com/coin-or/Cgl/wiki/CglResidualCapacity <current: off>
...
Value 'on' enables the cut generator and CBC will try it in the branch and cut tree (see cutDepth on how to fine tune the behavior). Value 'root' lets CBC run the cut generator generate only at the root node. Value 'ifmove' lets CBC use the cut generator in the tree if it looks as if it is doing some good and moves the objective value. Value 'forceon' turns on the cut generator and forces CBC to use it at every node. Reference: https://github.com/coin-or/Cgl/wiki/CglTwomir <current: ifmove>

\textbf{Values:} \texttt{forceandglobal}, \texttt{forcelongon}, \texttt{onglobal}
...
Value 'on' enables the cut generator and CBC will try it in the branch and cut tree (see cutDepth on how to fine tune the behavior). Value 'root' lets CBC run the cut generator generate only at the root node. Value 'ifmove' lets CBC use the cut generator in the tree if it looks as if it is doing some good and moves the objective value. Value 'forceon' turns on the cut generator and forces CBC to use it at every node. This implementation was written by Alberto Caprara. onglobal <current: ifmove>
...
MixedIntegerRounding2 can work on constraints created by aggregating constraints in model.  Although the coding for this has been in for some time, it is being modified and the user may wish to play with this. -1 varies the level at various times. current 1>
...
The default is to do one pass. A negative value -n means that n passes are also applied if the objective does not drop. current 10>

\textbf{Range:} -2147483647 to 2147483647
...
Some cut generators are fairly slow - this limits the number of times they are tried. The cut generators identified as 'may be slow' at present are Lift and project cuts and both versions of Reduce and Split cuts. current 10>
...
If nonnegative, ZeroHalf skips any candidate row whose weakening pair count exceeds this threshold. Negative values disable the filter. current 150000>
...
\textbf{Values:} \texttt{off}, \texttt{on}, \texttt{both}, \texttt{before}

\subsection*{\texttt{-Dins}}
\addcontentsline{toc}{subsection}{\texttt{-Dins}}

Whether to try Distance Induced Neighborhood Search

Value 'on' means to use the heuristic in each node of the tree, i.e. after preprocessing. Value 'before' means use the heuristic only if option doHeuristics is used. Value 'both' means to use the heuristic if option doHeuristics is used and during solve. <current: off>

\textbf{Values:} \texttt{off}, \texttt{on}, \texttt{both}, \texttt{before}, \texttt{often}

\subsection*{\texttt{-DivingCoefficient}}
\addcontentsline{toc}{subsection}{\texttt{-DivingCoefficient}}

Whether to try Coefficient diving heuristic

Value 'on' means to use the heuristic in each node of the tree, i.e. after preprocessing. Value 'before' means use the heuristic only if option doHeuristics is used. Value 'both' means to use the heuristic if option doHeuristics is used and during solve. <current: on>

\textbf{Values:} \texttt{off}, \texttt{on}, \texttt{both}, \texttt{before}

\subsection*{\texttt{-DivingFractional}}
\addcontentsline{toc}{subsection}{\texttt{-DivingFractional}}
...
\subsection*{\texttt{-DivingGuided}}
\addcontentsline{toc}{subsection}{\texttt{-DivingGuided}}
...
\subsection*{\texttt{-DivingLineSearch}}
\addcontentsline{toc}{subsection}{\texttt{-DivingLineSearch}}
...
\subsection*{\texttt{-DivingPseudocost}}
\addcontentsline{toc}{subsection}{\texttt{-DivingPseudocost}}
...
\subsection*{\texttt{-DivingSome}}
\addcontentsline{toc}{subsection}{\texttt{-DivingSome}}
...
\subsection*{\texttt{-DivingVectorLength}}
\addcontentsline{toc}{subsection}{\texttt{-DivingVectorLength}}
...
\subsection*{\texttt{-dwHeuristic}}
\addcontentsline{toc}{subsection}{\texttt{-dwHeuristic}}

Whether to try Dantzig Wolfe heuristic

This heuristic is very very compute intensive. It tries to find a Dantzig Wolfe structure and use that. Value 'on' means to use the heuristic in each node of the tree, i.e. after preprocessing. Value 'before' means use the heuristic only if option doHeuristics is used. Value 'both' means to use the heuristic if option doHeuristics is used and during solve. trial <current: off>

\textbf{Values:} \texttt{off}, \texttt{on}, \texttt{both}, \texttt{before}, \texttt{special}

\subsection*{\texttt{-feasibilityPump}}
\addcontentsline{toc}{subsection}{\texttt{-feasibilityPump}}

Whether to try Feasibility Pump

This switches on feasibility pump heuristic at root. This is due to Fischetti and Lodi and uses a sequence of LPs to try and get an integer feasible solution.  Some fine tuning is available by passFeasibilityPump.Value 'on' means to use the heuristic in each node of the tree, i.e. after preprocessing. Value 'before' means use the heuristic only if option doHeuristics is used. Value 'both' means to use the heuristic if option doHeuristics is used and during solve. <current: on>

\textbf{Values:} \texttt{off}, \texttt{on}, \texttt{both}, \texttt{before}

\subsection*{\texttt{-greedyHeuristic}}
\addcontentsline{toc}{subsection}{\texttt{-greedyHeuristic}}
...
\subsection*{\texttt{-heuristicsOnOff}}
\addcontentsline{toc}{subsection}{\texttt{-heuristicsOnOff}}

Switches most heuristics on or off

This can be used to switch on or off all heuristics.  Then you can set individual ones off or on.  CbcTreeLocal is not included as it dramatically alters search. <current: on>

\textbf{Values:} \texttt{off}, \texttt{on}, \texttt{both}, \texttt{before}

\subsection*{\texttt{-localTreeSearch}}
\addcontentsline{toc}{subsection}{\texttt{-localTreeSearch}}

Whether to use local tree search

This switches on a local search algorithm when a solution is found. This is from Fischetti and Lodi and is not really a heuristic although it can be used as one. When used from this program it has limited functionality. <current: off>

\textbf{Values:} \texttt{off}, \texttt{on}, \texttt{10}, \texttt{100}, \texttt{300}

\subsection*{\texttt{-naiveHeuristics}}
\addcontentsline{toc}{subsection}{\texttt{-naiveHeuristics}}
...
\subsection*{\texttt{-pivotAndFix}}
\addcontentsline{toc}{subsection}{\texttt{-pivotAndFix}}
...
\subsection*{\texttt{-proximitySearch}}
\addcontentsline{toc}{subsection}{\texttt{-proximitySearch}}
...
\textbf{Values:} \texttt{100}, \texttt{300}

\subsection*{\texttt{-randomizedRounding}}
\addcontentsline{toc}{subsection}{\texttt{-randomizedRounding}}
...
\subsection*{\texttt{-Rens}}
\addcontentsline{toc}{subsection}{\texttt{-Rens}}

Whether to try Relaxation Enforced Neighborhood Search

Value 'on' means to use the heuristic in each node of the tree, i.e. after preprocessing. Value 'before' means use the heuristic only if option doHeuristics is used. Value 'both' means to use the heuristic if option doHeuristics is used and during solve. Value 'on' just does 50 nodes. 200, 1000, and 10000 does that many nodes. <current: off>

\textbf{Values:} \texttt{1000}, \texttt{10000}, \texttt{dj}, \texttt{djbefore}, \texttt{usesolution}

\subsection*{\texttt{-Rins}}
\addcontentsline{toc}{subsection}{\texttt{-Rins}}
...
\subsection*{\texttt{-roundingHeuristic}}
\addcontentsline{toc}{subsection}{\texttt{-roundingHeuristic}}

Whether to use Rounding heuristic

This switches on a simple (but effective) rounding heuristic at each node of tree. <current: on>

\textbf{Values:} \texttt{off}, \texttt{on}, \texttt{both}, \texttt{before}

\subsection*{\texttt{-doHeuristic}}
\addcontentsline{toc}{subsection}{\texttt{-doHeuristic}}

Do heuristics before any preprocessing

Normally heuristics are done in branch and bound.  It may be useful to do them outside. Only those heuristics with 'both' or 'before' set will run. Doing this may also set cutoff, which can help with preprocessing. <current: off>

\textbf{Values:} \texttt{off}, \texttt{on}

\subsection*{\texttt{-forceSolution}}
\addcontentsline{toc}{subsection}{\texttt{-forceSolution}}

Whether to use given solution as crash for BAB

If on then tries to branch to solution given by AMPL or priorities file. <current: off>

\textbf{Values:} \texttt{off}, \texttt{on}

\subsection*{\texttt{-depthMiniBab}}
\addcontentsline{toc}{subsection}{\texttt{-depthMiniBab}}

Depth at which to try mini branch-and-bound

Rather a complicated parameter but can be useful. -1 means off for large problems but on as if -12 for problems where rows+columns<500, -2 means use Cplex if it is linked in.  Otherwise if negative then go into depth first complete search fast branch and bound when depth>= -value-2 (so -3 will use this at depth>=1).  This mode is only switched on after 500 nodes.  If you really want to switch it off for small problems then set this to -999.  If >=0 the value doesn't matter very much.  The code will do approximately 100 nodes of fast branch and bound every now and then at depth>=5. The actual logic is too twisted to describe here. The default has been changed from -1 to +1.  This uses Clp and saves factorizations etc to be faster. current 1>

\textbf{Range:} -2147483647 to 2147483647

\subsection*{\texttt{-diveOpt}}
\addcontentsline{toc}{subsection}{\texttt{-diveOpt}}

Diving options

If >2 \&\& <=8 then modify diving options - 3 only at root and if no solution, 4 only at root and if this heuristic has not got solution, 5 decay only if no solution, 6 if depth <3 or decay, 7 run up to 2 times if solution found 4 otherwise, 8 fire at every node until first incumbent then revert to default d\textasciicircum{}2/2\textasciicircum{}d schedule (aggressive feasibility mode), >10 All only at root (DivingC normal as value-10), >20 All with value-20). current 2>

\textbf{Range:} -1 to 20

\subsection*{\texttt{-diveSolves}}
\addcontentsline{toc}{subsection}{\texttt{-diveSolves}}

Diving solve option

If >0 then do up to this many solves. However, the last digit is ignored and used for extra options: 1-3 enables fixing of satisfied integer variables (but not at bound), where 1 switches this off for that dive if the dive goes infeasible, and 2 switches it off permanently if the dive goes infeasible. current 100>

\textbf{Range:} -1 to 200000

\subsection*{\texttt{-passFeasibilityPump}}
\addcontentsline{toc}{subsection}{\texttt{-passFeasibilityPump}}

How many passes in feasibility pump

This fine tunes the Feasibility Pump heuristic by doing more or fewer passes. current 30>

\textbf{Range:} 0 to 10000

\subsection*{\texttt{-pumpTune}}
\addcontentsline{toc}{subsection}{\texttt{-pumpTune}}

Dubious ideas for feasibility pump

This fine tunes Feasibility Pump >=10000000 use as objective weight switch >=1000000 use as accumulate switch >=1000 use index+1 as number of large loops ==100 use objvalue +0.05*fabs(objvalue) as cutoff OR fakeCutoff if set \%100 == 10,20 affects how each solve is done 1 == fix ints at bounds, 2 fix all integral ints, 3 and continuous at bounds. If accumulate is on then after a major pass, variables which have not moved are fixed and a small branch and bound is tried. current 1005043>

\textbf{Range:} 0 to 1000000000

\subsection*{\texttt{-hOptions}}
\addcontentsline{toc}{subsection}{\texttt{-hOptions}}
...
\subsection*{\texttt{-fpumpPassFreq}}
\addcontentsline{toc}{subsection}{\texttt{-fpumpPassFreq}}

Print feasibility pump progress every N passes (0 = disabled).

Print feasibility pump progress every N passes (0 = disabled). current 0>

\textbf{Range:} 0 to 1000000

\subsection*{\texttt{-artificialCost}}
\addcontentsline{toc}{subsection}{\texttt{-artificialCost}}

Costs >= this treated as artificials in feasibility pump

Costs >= this treated as artificials in feasibility pump current 0>

\textbf{Range:} 0 to 1.79769e+308
...
fastPreProcess : Run fast MILP preprocessing on the loaded model fastPreProcessL(evel) : Aggression level for fast MILP preprocessing before solve fastPreProcessM(axRounds) : Maximum number of bound-tightening rounds in fast preprocessing
...
Normally the preprocessed model has column names replaced by new names C0000... Setting this option to on keeps original names in variables which still exist in the preprocessed problem <current: on>
...
When on, singleton rows (rows with a single nonzero) are used to tighten variable bounds before the initial LP solve and conflict graph construction. This is a cheap preprocessing step that can fix variables and reduce the problem size. <current: on>
...
Normally if AMPL says there are SOS variables they should be used, but sometimes they should be turned off - this does so. <current: off>
...
Whether and when to perform Clique Strengthening preprocessing routine <current: after>

\textbf{Values:} \texttt{off}, \texttt{before}, \texttt{after}
...
Controls how aggressively fast preprocessing tightens variable bounds before the initial LP solve. off:       disabled (falls back to singletonBounds setting). singletons: singleton rows only --- same as singletonBounds on. milpbt:    singletons then knapsack-based bound tightening for up to fastPreProcessMaxRounds rounds (default 100, effectively fixpoint). fixpoint:  singletons then bound tightening until no new fixings are found, regardless of fastPreProcessMaxRounds. <current: milpbt>

\textbf{Values:} \texttt{off}, \texttt{singletons}, \texttt{milpbt}, \texttt{fixpoint}
...
Once you like what the stand-alone solver does then this allows you to generate user\_driver.cpp which approximates the code. 0 gives simplest driver, 1 generates saves and restores, 2 generates saves and restores even for variables at default value. 4 bit in cbc generates size dependent code rather than computed values. This is now deprecated as you can call stand-alone solver - see Cbc/examples/driver4.cpp. current 0> Once you like what the stand-alone solver does then this allows you to generate user\_driver.cpp which approximates the code.  0 gives simplest driver, 1 generates saves and restores, 2 generates saves and restores even for variables at default value. 4 bit in cbc generates size dependent code rather than computed values. current 0>
...
Switches on a trivial re-formulation that introduces extra integer variables to group together variables with same cost. current 0>
...
If set, integer variables with reduced costs greater than the specified value will be fixed before branch and bound - use with extreme caution! current 0>

\textbf{Range:} -1e+20 to 1e+20
...
This sleazy trick can help on some problems. current 0>
...
Presolve analyzes the model to find such things as redundant equations, equations which fix some variables, equations which can be transformed into bounds, etc. For the initial solve of any problem this is worth doing unless one knows that it will have no effect. Option 'on' will normally do 5 passes, while using 'more' will do 10.  If the problem is very large one can let CLP write the original problem to file by using 'file'. <current: on>

\textbf{Values:} \texttt{on}, \texttt{off}, \texttt{more}, \texttt{file}
...
0 - off, 1 choose blocks >1 use as blocks Dantzig Wolfe if primal, Benders if dual - uses sprint pass for number of passes current 0>
...
Don't even think about it. current 3>
...
Normally Presolve does 10 passes but you may want to do less to make it more lightweight or do more if improvements are still being made. As Presolve will return if nothing is being taken out, you should not normally need to use this fine tuning. current 5>

\textbf{Range:} -200 to 100
...
\textbf{Values:} \texttt{updepth}, \texttt{downdepth}
...
This switches on Orbital branching. Value 'on' just adds orbital, 'strong' tries extra fixing in strong branching.'cuts' just adds global cuts to break symmetry.'lightweight' is as on where computation seems cheap <current: off>

\textbf{Values:} \texttt{on}, \texttt{lightweight}, \texttt{moreprinting}, \texttt{cuts}, \texttt{cutslight}
...
This sets priorities for SOS.  Values 'high' and 'low' just set a priority relative to the for integer variables.  Value 'orderhigh' gives first highest priority to the first SOS and integer variables a low priority.  Value 'orderlow' gives integer variables a high priority then SOS in order. <current: off>

\textbf{Values:} \texttt{off}, \texttt{high}, \texttt{low}, \texttt{orderhigh}, \texttt{orderlow}
...
Using strong branching computes pseudo-costs.  After this many times for a variable we just trust the pseudo costs and do not do any more strong branching. current 10>

\textbf{Range:} -3 to 2147483647
...
In order to decide which variable to branch on, the code will choose up to this number of unsatisfied variables and try mini up and down branches.  The most effective one is chosen. If a variable is branched on many times then the previous average up and down costs may be used - see number before trust. current 5>

\textbf{Range:} 0 to 999999
...
A primitive way of deciding which node to explore next.  Satisfying each integer infeasibility is expected to cost this much. current 0>
...
When checking a solution for feasibility, if the difference between the value of a variable and the nearest integer is less than the integer tolerance, the value is considered to be integral. Beware of setting this smaller than the primal tolerance. current 1e-06>

\textbf{Range:} -20 to 0.5
...
One may want to increase this tolerance if presolve says the problem is infeasible and one has awkward numbers and is sure that the problem is really feasible. current 1e-08>
...
Normally the default tolerance is fine, but one may want to increase it a bit if the primal simplex algorithm seems to be having a hard time. current 1e-06>
...
This applies to reading mps files (and also lp files if KILL\_ZERO\_READLP defined) current 1e-20>

\textbf{Range:} -100 to 1e
...
This switches the conflict graph-based preprocessing and cut separation routines (CglBKClique, CglOddWheel and CliqueStrengthening) on or off. Values: off: turns these routines off; on: turns these routines on; clq: turns these routines off and enables the cut separator of CglClique. <current: on>

\textbf{Values:} \texttt{off}, \texttt{on}, \texttt{clq}
...
Pivoting strategy used in Bron-Kerbosch algorithm current 3>
...
Maximum number of recursive calls made by Bron-Kerbosch algorithm current 1000>
...
Sets the method used in the extension module of BK Clique Cut Separation routine: 0=no extension; 1=random; 2=degree; 3=modified degree; 4=reduced cost(inversely proportional); 5=reduced cost(inversely proportional) + modified degree current 4>
...
Sets the method used in the extension module of Odd Wheel Cut Separation routine: 0=no extension; 1=one variable; 2=clique current 2>
...
Defines how adventurous you want to be in using new ideas. 0 then no new ideas, 1 fairly sensible, 2 a bit dubious, 3 you are on your own! current 0>
...
Maximum iterations on hot start current 100>
...
If set Or's with specialOptions just before entering branchAndBound. current 0>
...
A value of 0.0 means off. Otherwise, add a constraint forcing objective below this value in feasibility pump current 0>

\textbf{Range:} -1.79769e+308 to 1.79769e+308
...
After a pass in the feasibility pump, variables which have not moved about are fixed and if the preprocessed model is smaller than this fraction of the original problem, a few nodes of branch and bound are done on the reduced problem. current 0.5>

\textbf{Range:} -05 to 1.1
...
All bounds <= this value - DEBUG current 0>

\textbf{Range:} 1 to 1e+15
...
Controls which LP algorithm is used when -solve or -initialSolve triggers the root LP relaxation. dual:    dual simplex (default). primal:  primal simplex. barrier: interior-point (barrier) method. <current: dual>

\textbf{Values:} \texttt{dual}, \texttt{primal}, \texttt{barrier}
...
Number of solutions to save. current 10>
...
Allows initialization of the random seed for pseudo-random numbers used in heuristics such as the Feasibility Pump to decide whether to round up or down. The special value of 0 lets Cbc use the time of the day for the initial seed. current 42>
...
The default is minimize - use 'direction maximize' for maximization. You can also use the parameters 'maximize' or 'minimize'. min(imize)  max(imize)  zero <current: min(imize)> The default is minimize - use 'direction maximize' for maximization. You can also use the parameters\_ 'maximize' or 'minimize'. max(imize)  min(imize)  zero <current: min(imize)>
...
Useful for testing if maximization works correctly Useful for testing if maximization works correctly
...
If this is on ClpPackedMatrix uses extra column copy in odd format. <current: off>
...
Initialization of the random seed for pseudo-random numbers used to break ties in degenerate problems. This may yield a different continuous optimum and, in the context of Cbc, different cuts and heuristic solutions. The special value of 0 lets CLP use the time of the day for the initial seed. current 1234567>
...
Whether to use KKT factorization in barrier <current: off>
...
Perturbation helps to stop cycling, but CLP uses other measures for this. However, large problems and especially ones with unit elements and unit right hand sides or costs benefit from perturbation.  Normally CLP tries to be intelligent, but one can switch this off. <current: on>
...
The Dantzig method is simple but its use is deprecated.  Steepest is the method of choice and there are two variants which keep all weights updated but only scan a subset each iteration. Partial switches this on while automatic decides at each iteration based on information about the factorization. The PE variants add the Positive Edge criterion. This selects incoming variables to try to avoid degenerate moves. See also option psi. auto(matic)  dant(zig)  partial  steep(est)  PEsteep(est) PEdantzig <current: auto(matic)>
...
The default is to use the normal CoinFactorization, but other choices are a dense one, OSL's, or one designed for small problems. <current: normal>

\textbf{Values:} \texttt{normal}, \texttt{dense}, \texttt{simple}, \texttt{osl}
...
The Dantzig method is simple but its use is deprecated.  Exact devex is the method of choice and there are two variants which keep all weights updated but only scan a subset each iteration. Partial switches this on while 'change' initially does 'dantzig' until the factorization becomes denser. This is still a work in progress. The PE variants add the Positive Edge criterion. This selects incoming variables to try to avoid degenerate moves. See also Towhidi, M., Desrosiers, J., Soumis, F., The positive edge criterion within COIN-OR's CLP; Omer, J., Towhidi, M., Soumis, F., The positive edge pricing rule for the dual simplex. auto(matic)  exa(ct)  dant(zig)  part(ial)  steep(est) change  sprint  PEsteep(est)  PEdantzig <current: auto(matic)>
...
If processed problem <= this use dense factorization current -1>

\textbf{Range:} -1 to 10000
...
This is a type of 'crash' which works well on some homogeneous problems. It works best on problems with unit elements and rhs but will do something to any model.  It should only be used before the primal simplex algorithm. It can be set to -1 when the code decides for itself whether to use it, 0 to switch off, or n > 0 to do n passes. current -1>
...
If this is left at its default value of 200 then CLP will guess a value to use.  CLP may decide to re-factorize earlier for accuracy. current 200>
...
This can be used for testing purposes.  The corresponding library call setMaximumIterations(value) can be useful.  If the code stops on seconds or by an interrupt this will be treated as stopping on maximum iterations. This is ignored in branchAndCut - use maxN!odes. current 2147483647>
...
See ClpSimplex.hpp. Alternative syntax - kwd1+kwd2+1234 - - keep(DualOrPrimal) (If you ask for dual you will always get dual (and for primal)) -> 8192 - clean(Scaled) (Make sure unscaled problem is feasible if scaled problem is feasible) -> 134217728 current 0>
...
Method of perturbation current 50>

\textbf{Range:} -5000 to 102
...
If you are solving a quadratic problem using primal then it may be helpful to do some sequential Lps to get a good approximate solution. current 0>

\textbf{Range:} -50000 to 50000
...
If processed problem <= this use small factorization current -1>
...
Dubious options for Simplex - see ClpSimplex.hpp current 0>
...
For a barrier code to be effective it needs a good Cholesky ordering and factorization. The native ordering and factorization is not state of the art, although acceptable. You may want to link in one from another source.  See Makefile.locations for some possibilities. native  dense  fudge(Long\_dummy)  wssmp\_dummy  Uni(versityOfFlorida\_dummy) <current: native>

\textbf{Values:} \texttt{Taucs\_dummy}, \texttt{Mumps\_dummy}, \texttt{Pardiso\_dummy}
...
Whether to regularize barrier <current: off>

\textbf{Values:} \texttt{gammastrong}, \texttt{deltastrong}
...
If the objective function has some very large values, you may wish to scale them internally by this amount.  It can also be set by autoscale. It is applied after scaling.  You are unlikely to need this. current 1>
...
You can set this to -1.0 to test maximization or other to stress code current 1>
...
If the rhs or bounds have some very large meaningful values, you may wish to scale them internally by this amount.  It can also be set by autoscale.  This should not be needed. current 1>
...
If set then only those names which match mask are printed in a solution. '?' matches any character and '*' matches any set of characters. The default is '' (unset) so all variables are printed. This is only active if model has names. <Current value is (unset)>
...
Precision: \%.nf -> n digits after decimal; \%.ng -> n significant digits; Width: \%mw -> minimum field width, padded with spaces by default. Remember the f or g at end as \%18.5 by itself gives garbage. <Current value is `\%15.8g'>
...
Checking the time costs more than one might think. In cbc one does not normally need to stop after generating a cut or doing an iteration. So less checks less often and often is more likely to check every iteration. <current: often>

\textbf{Values:} \texttt{less}, \texttt{often}
...
This changes the amount and format of printing a solution: normal - nonzero column variables integer - nonzero integer column variables special - in format suitable for OsiRowCutDebugger rows - nonzero column variables and row activities all - all column variables and row activities. For non-integer problems 'integer' and 'special' act like 'normal'. Also see printMask for controlling output. csv  bound(ranging)  rhs(ranging)  objective(ranging)  stats <current: normal> This changes the amount and format of printing a solution: normal - nonzero column variables integer - nonzero integer column variables special - in format suitable for OsiRowCutDebugger rows - nonzero column variables and row activities all - all column variables and row activities. For non-integer problems 'integer' and 'special' act like 'normal'. Also see printMask for controlling output. csv  bound(ranging)  rhs(ranging)  objective(ranging)  stats <current: normal>

\textbf{Values:} \texttt{boundsint}, \texttt{boundsall}, \texttt{fixint}, \texttt{fixall}, \texttt{allcsv}
...
elapsed uses elapsed (wall-clock) time for stopping, while cpu uses CPU time. Elapsed is the default as it is more natural for users. (On Windows, elapsed time is always used). <current: elapsed>

\textbf{Values:} \texttt{cpu}, \texttt{elapsed}
...
If set to 0 then there should be no output in normal circumstances. A value of 1 is probably the best value for most uses, while 2 and 3 give more information. current 1>

\textbf{Range:} -1 to 999999
...
When set to 1 (default), each output line is flushed immediately. This is already the default behaviour of CoinMessageHandler (which calls fflush after every CoinMessageEol). Setting to 0 is reserved for future use to allow batching of output for performance in non-interactive scenarios. current 0>
...
Controls whether UTF-8 characters ($\in$, $\kappa$, ---) are used in solver output. -1 (default) auto-detects from the locale (LANG/LC\_ALL environment variables). 0 forces ASCII-only output. 1 forces UTF-8 output. current 0>
...
When solving the LP relaxation at the root node, print a progress row every N iterations. Set to 0 to disable iteration-based printing. Use lpTimeFreq for time-based printing. current 0>
...
If this is greater than 0 then presolve will give more information and branch and cut will give statistics current 0>
...
When solving the LP relaxation at the root node, print a progress row every N seconds. Set to 0 to disable time-based printing. Use lpIterFreq for iteration-based printing. current 5>
...
Print feasibility pump progress every N seconds (0 = disabled, default 5). current 5>

\textbf{Range:} 0 to 1e+10
...
Default is on, off switches on unbuffered output <current: on>
...
For the sake of your sanity, only the more useful and simple commands are printed out on ?. <current: more> For the sake of your sanity, only the more useful and simple commands are printed out on ?. <current: more>

\textbf{Values:} \texttt{more}, \texttt{important}, \texttt{all}
...
This sets a minimum interval for some printing - elapsed seconds current 0.7>
...
export : Export model as mps file exportF(ile) : sets name for file to export model to
...
import : Import model from file importF(ile) : sets name for file to import model from
...
Sets the file name used by writeFeatures. If name is not specified the previous value is used. Initialized to 'features.csv'. The header row listing all 207 feature names is written automatically when the file is new or empty; subsequent calls append a new row. <Current value is `features.csv'>
...
This appends statistics to given file name.  If name is not specified, the previous value will be used. This is initialized to '', i.e. it must be set. Adds header if file empty or does not exist. <Current value is `stats.csv'>
...
This will set the name of the model will be written to and read from. This is initialized to 'export.mps'. <Current value is `export.mps'>
...
This will set the name of the model to be read in with the import command. This is initialized to 'import.mps' <Current value is `import.mps'>
...
This will set the name the GMPL solution will be written to and read from. This is initialized to 'gmpl.sol'. <Current value is `gmpl.sol'>
...
This will set the name the model will be written to and read from. This is initialized to 'prob.mod'. <Current value is `mipstart.sol'>
...
This will set the name the model will be written to and read from. This is initialized to 'prob.mod'. <Current value is `prob.mod'>
...
This will set the name solutions will be written to and read from. This is initialized to 'next.sol'. <Current value is `next.sol'>
...
This will set the name the solution will be saved to and read from. By default, solutions are written to 'opt.sol'. To print to stdout, use printSolution. <Current value is `opt.sol'>
...
This will set the name the solution will be saved to and read from. By default, binary solutions are written to 'solution.file'.use printSolution. <Current value is `solution.file'>
...
This sets the directory which import, export, saveModel, restoreModel etc. will use. It is initialized to the current directory. <Current value is (unset)>
...
This sets the directory where the COIN-OR sample problems reside. It is used only when -unitTest is passed to cbc. cbc will pick up the test problems from this directory. <Current value is (unset)>
...
This will read an MPS format basis file from the given file name. It will use the default directory given by 'directory'.  If no name is specified, the previous value will be used. This is initialized to '', i.e. it must be set.  If you have libz then it can read compressed files 'xxxxxxxx.gz' or xxxxxxxx.bz2. <Current value is `default.bas'>
...
This will read a file with parametric data from the given file name and then do parametrics. It will use the default directory given by 'directory'. A name of '\$' will use the previous value for the name. This is initialized to '', i.e. it must be set.  This can not read from compressed files. File is in modified csv format - a line ROWS will be followed by rows data while a line COLUMNS will be followed by column data.  The last line should be ENDATA. The ROWS line must exist and is in the format ROWS, inital theta, final theta, interval theta, n where n is 0 to get CLPI0062 message at interval or at each change of theta and 1 to get CLPI0063 message at each iteration. If interval theta is 0.0 or >= final theta then no interval reporting. n may be missed out when it is taken as 0.  If there is Row data then there is a headings line with allowed headings - name, number, lower(rhs change), upper(rhs change), rhs(change).  Either the lower and upper fields should be given or the rhs field. The optional COLUMNS line is followed by a headings line with allowed headings - name, number, objective(change), lower(change), upper(change). Exactly one of name and number must be given for either section and missing ones have value 0.0. <Current value is `default.mps'>
...
It saves space to get rid of names so if you need to you can set this to off. This needs to be set before the import of model - so -keepnames off -import xxxxx.mps. <current: on>
...
To use multiple threads, set threads to number wanted.  It may be better to use one or two more than number of cpus available.  If 100+n then n threads and search is repeatable (maybe be somewhat slower), if 200+n use threads for root cuts, 400+n threads used in sub-trees. current 0>

\textbf{Range:} -100 to 100000
