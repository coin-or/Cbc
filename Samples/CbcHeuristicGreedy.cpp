...
// Default Constructor
CbcGreedyEquality::CbcGreedyEquality() 
  :CbcHeuristic()
{
  // matrix  will automatically be empty
...
// Constructor from model
CbcGreedyEquality::CbcGreedyEquality(CbcModel & model)
  :CbcHeuristic(model)
{
  // Get a copy of original matrix
  assert(model.solver());
...
// Clone
CbcHeuristic *
CbcGreedyEquality::clone() const
{
  return new CbcGreedyEquality(*this);
}

// Copy constructor 
CbcGreedyEquality::CbcGreedyEquality(const CbcGreedyEquality & rhs)
:
  CbcHeuristic(rhs),
  matrix_(rhs.matrix_),
...
// Assignment operator 
CbcGreedyEquality & 
CbcGreedyEquality::operator=( const CbcGreedyEquality& rhs)
{
  if (this != &rhs) {
    setWhen(rhs.when());
    matrix_=rhs.matrix_;
...
// Returns 1 if solution, 0 if not
int
CbcGreedyEquality::solution(double & solutionValue,
			 double * betterSolution)
{
  if (!model_)
    return 0;
...
// update model
void CbcGreedyEquality::setModel(CbcModel * model)
{
#define SLOPPY
#ifndef SLOPPY
  model_ = model;
  assert(model_->solver());
  *this = CbcGreedyEquality(*model); 
#else
  if (model_&&model!=model_) {
    model_ = model;
    assert(model_->solver());
...
// Resets stuff if model changes
void 
CbcGreedyEquality::resetModel(CbcModel * model)
{
#ifndef SLOPPY
  model_ = model;
  assert(model_->solver());
