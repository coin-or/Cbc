  ...
  */
  ...
  double * coeffMultiplier=NULL;
  {
	...
	numberIntegerObj*5<numberObjects_&&!parentModel_&&
      ...
      iType=2;
    //else if (!numberContinuousObj&&
    //numberIntegerObj*20<numberObjects_&&!parentModel_&&
    //     solver_->getNumRows()>100) 
    //iType=8;
...
*/
  ...
  if (solver_->getRowCutDebuggerAlways()) {
#ifdef CHECK_PATH
    if (numberColumns_Z==-1) {
      // If not -1 then in nested B&B
	...
	const_cast<OsiRowCutDebugger *> (solver_->getRowCutDebuggerAlways());
      ...
      numberColumns_Z = getNumCols();
    }
  } else {
    debuggerSolution_Z = NULL;
    numberColumns_Z = -1;
#else
    specialOptions_ |= 1;
#endif
...

{
  ...
  if (!solverCharacteristics_->solutionAddsCuts()) {
    ...
    */
    ...
    if (saveObjectiveValue+1.0e-5<objectiveValue) {
#if COIN_DEVELOP>1
  ...
  if (!retCode) {
    ...
    */
	...
	for (int i=0;i<100;i++)
      ...
      OsiClpSolverInterface * clpSolver 
	= dynamic_cast<OsiClpSolverInterface *> (solver_);
      if (clpSolver&&fastNodeDepth_<-1) {
#define FATHOM_BIAS 1
	//#ifdef COIN_DEVELOP
	if (numberNodes_==1) {
	...
	*/
	...
	if (node->depth()>=go_fathom &&!parentModel_
	  ...
	  fillPseudoCosts(down,up,numberDown,numberUp,
...
{
#if 0
  CbcObject * object = dynamic_cast<CbcObject *> (data.object_);
  bool found=false;
  for (int j=0;j<numberObjects_;j++) {
    if (object== object_[j]) {
      found=true;
      if (j!=data.objectNumber_) {
	printf("bad number\n");
      }	
      break;
    }
		     ...
		     int numberNodes)
{
  ...
  while (!tree_->empty()) {
	...
	assert (object);
	// could use variable_??
	int columnNumber = object->columnNumber();
	assert (columnNumber>=0);
	columnNumber = back[columnNumber];
	assert (columnNumber>=0);
	CbcObject * objectNew = dynamic_cast<CbcObject *> (otherModel->object_[columnNumber]);
...
CbcModel::whileIterating(int numberIterations)
{
  ...
  while (true) {
    ...
    if (stopNumberIterations_>0&&numberIterations_>stopNumberIterations_) {
      // out of loop
      break;
    }
    /*
...
CbcModel::doCutsNow(int allowForTopOfTree) const
{
  ...
  if (whenCuts_<0||(size<=500&&allowForTopOfTree!=3)) {
