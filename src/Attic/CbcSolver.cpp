...
CbcSolverUsefulData::CbcSolverUsefulData()
{
  ...
  for (int i = 0; i < (int)parameters_.size(); i++) {
    printf("%d param type %d name %s\n",i,parameters_[i].type(),
	   parameters_[i].matchName().c_str());
  }
  more2 = parameters_[whichParam(CBC_PARAM_INT_MOREMOREMIPOPTIONS, parameters_)].intValue();
  printf("more2 %d\n",more2);
#endif
}
...
#define MORE_CUTS
          ...
          case CBC_PARAM_ACTION_BAB: // branchAndBound
			...
			delete [] solution2;
              ...
              if (mipStartBefore.size()) {
                ...
                double obj;
                int status = CbcMipStart::computeCompleteSolution(&tempModel, tempModel.solver(), colNames, mipStartBefore, &x[0], obj, 0, tempModel.messageHandler(), tempModel.messagesPointer());
    ...
    clqstrAction = "off";
		...
		babModel_->solver()->activateRowCutDebugger(problemName.c_str());
              ...
              int accuracyFlag[30] = {};
              char doAtEnd[30] = {};
	      int lagrangeanFlag = (parameters_[whichParam(CBC_PARAM_INT_MOREMOREMIPOPTIONS,parameters_)].intValue()&(7*33554432))>>9;
              ...
              if (type == CBC_PARAM_ACTION_BAB) {
                ...
                if ((mipStart.size() || testEmpty == "empty.") && !mipStartBefore.size()
                  ...
                  if (extraActions == 6)
                    ...
                    extraActions++;
                  int status = CbcMipStart::computeCompleteSolution(babModel_, babModel_->solver(), colNames, mipStart, &x[0], obj, extraActions, babModel_->messageHandler(), babModel_->messagesPointer());
                  if (!status) {
...
#undef COINUTILS_HAS_GLPK
          ...
          case CBC_PARAM_ACTION_MIPSTART:
            if (goodModel) {
              ...
              CbcMipStart::read(model_.solver(), fileName.c_str(), mipStart, msObj, model_.messageHandler(), model_.messagesPointer());
...
clp watson.mps -\nscaling off\nprimalsimplex");
		...
		const_cast<char *>(model2->guess(1).c_str());
