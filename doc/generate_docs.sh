...
#   doc/cbc-parameters.md  — Markdown reference
...
python3 "$SCRIPT_DIR/generate_html_reference.py" "$CBC" > "$SCRIPT_DIR/../docs/parameters.html"

if command -v pdflatex >/dev/null 2>&1; then
    echo "Compiling PDF..."
    TMPDIR=$(mktemp -d)
    PDFLOG="$TMPDIR/pdflatex.log"
    if pdflatex -interaction=nonstopmode -output-directory="$TMPDIR" "$SCRIPT_DIR/cbc-parameters.tex" >"$PDFLOG" 2>&1 && \
       pdflatex -interaction=nonstopmode -output-directory="$TMPDIR" "$SCRIPT_DIR/cbc-parameters.tex" >>"$PDFLOG" 2>&1; then
...
echo ""
echo "Generated:"
ls -lh "$SCRIPT_DIR/cbc.1" "$SCRIPT_DIR/cbc-parameters.tex" \
       "$SCRIPT_DIR/cbc-parameters.md" "$SCRIPT_DIR/cbc-parameters.pdf" \
