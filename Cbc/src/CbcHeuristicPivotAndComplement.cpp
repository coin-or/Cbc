/* $Id: CbcHeuristicPivotAndComplement.cpp 1200 2009-07-25 08:44:13Z forrest $ */
...
// Default Constructor
CbcHeuristicPivotAndComplement::CbcHeuristicPivotAndComplement() 
  :CbcHeuristic()
{
}
...
CbcHeuristicPivotAndComplement::CbcHeuristicPivotAndComplement(CbcModel & model)
  :CbcHeuristic(model)
{
}
...
// Clone
CbcHeuristic *
CbcHeuristicPivotAndComplement::clone() const
{
  return new CbcHeuristicPivotAndComplement(*this);
}
// Create C++ lines to get to current state
void 
CbcHeuristicPivotAndComplement::generateCpp( FILE * fp) 
{
  CbcHeuristicPivotAndComplement other;
  fprintf(fp,"0#include \"CbcHeuristicPivotAndComplement.hpp\"\n");
  fprintf(fp,"3  CbcHeuristicPivotAndComplement heuristicPFX(*cbcModel);\n");
  CbcHeuristic::generateCpp(fp,"heuristicPFX");
  fprintf(fp,"3  cbcModel->addHeuristic(&heuristicPFX);\n");
}

// Copy constructor 
CbcHeuristicPivotAndComplement::CbcHeuristicPivotAndComplement(const CbcHeuristicPivotAndComplement & rhs)
:
  CbcHeuristic(rhs)
{
}
...
void 
CbcHeuristicPivotAndComplement::resetModel(CbcModel * /*model*/)
{
...
int
CbcHeuristicPivotAndComplement::solution(double & /*solutionValue*/,
				  double * /*betterSolution*/)
{

  numCouldRun_++; // Todo: Ask JJHF what this for.
  std::cout << "Entering Pivot-and-Complement Heuristic" << std::endl; 

#ifdef FORNOW
  std::cout << "Lucky you! You're in the Pivot-and-Complement Heuristic" << std::endl;
  ...
  */
	  ...
	  if(solutions.numberUnsatisfied[k] <= fixThreshold)
	    {
	      ...
	      int numComplemented=0;

	      // Complement the first fixThreshold number of integer vars
	      // that are satisfied
	      for (int iColumn = 0 ; iColumn < numberColumns ; iColumn++)
		{
		  if (newSolver->isInteger(iColumn))
		      ...
		      if (fabs(value-intValue)<1.0e-5)
			{
			  ...
			  numComplemented++;
			  if(numComplemented>numInt-fixThreshold)
			    break;
			}
		    }		      
		}
	      printf("numComplemented: %d\n", numComplemented);
  ...
  std::cout << "See you soon! You're leaving the Pivot-and-Complement Heuristic" << std::endl;
 std::cout << std::endl;
...
// update model
void CbcHeuristicPivotAndComplement::setModel(CbcModel * )
{
