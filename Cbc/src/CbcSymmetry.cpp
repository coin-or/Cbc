...
int
CbcSymmetry::statsOrbits(CbcModel * model, int type) const
{
  char general[200];
  int returnCode=0;
...
int 
CbcSymmetry::largestOrbit(const double * lower, const double * upper) const
{
...
int 
CbcSymmetry::orbitalFixing(OsiSolverInterface * solver)
{
  int numberColumns = solver->getNumCols();
...
#define MULTIPLIER 1 
  ...
  canonG_ = NULL;
  if ((G_ == 0&&GSparse_ == 0) || lab_ == 0 || ptn_ == 0 || 
...
void
CbcNauty::addElement(int ix, int jx)
{
...
void
CbcNauty::deleteElement(int ix, int jx)
{
  // Right now die if bad index.  Can throw exception later
  assert(ix < n_ && jx < n_);
  set *gv = GRAPHROW(G_, ix, m_);
...
int
CbcNauty::getNumGenerators() const
{
...
void
CbcNauty::getVstat(double *v, int nv)
{
  assert(nv == n_);
...
void 
CbcNauty::setWriteAutoms(const std::string &fname)
{
...
void
CbcOrbitalBranchingObject::fix(OsiSolverInterface * solver,
...
{
  const CbcOrbitalBranchingObject* br =
    dynamic_cast<const CbcOrbitalBranchingObject*>(brObj);
  assert(!br);
  abort();
  return 0;
}
