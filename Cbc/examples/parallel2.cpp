// $Id: parallel2.cpp 1902 2013-04-10 16:58:16Z stefan $
...
//#############################################################################
// AS parallel.cpp but just one char * and dummy callBack
...
int main (int argc, const char *argv[])
{
  ...
  for (int iModel=0;iModel<numberModels;iModel++) {
    ...
    // If going in by default method you can use CBC_NO_INTERRUPT
    ...
    allData[iModel].args=args;
    for (int iArgc=lastArgPosition;iArgc<lastArgPosition+thisArgc;iArgc++) {
      strcpy(args,argv[iArgc]);
      args += strlen(argv[iArgc]);
      args[0]=' ';
      args++;
    }
    // add -quit
