 ...
 *
 * Tests for the j3041_1 instance (MIP-feasible, solved to optimality).
 *
 * Known values (cross-validated with an independent LP solver):
 *   LP relaxation (pure, all rows):       0.12389597644749
 *   LP without row 50 (resR(1,15)):       0.02498365505193
 *   LP without rows 23+50:               0.02498365505193
 *   MIP optimal:                          3.0 (integer)
 *
 * Instance structure: 344 rows, 801 cols
 *   736 binary variables, 33 general integer, 32 continuous
 *
 * Reoptimisation test:
 *   S1 = {row 50} (resR(1,15))  — binding at LP optimum
 *   S2 = {row 23} (resR(0,23))  — redundant once S1 is present
 *   Remove S1+S2  → LP = 0.02498365505193
 *   Add S1        → LP = 0.12389597644749  (S1 restores tightness)
 *   Add S2        → LP = 0.12389597644749  (= full LP, S2 redundant)
 */
...
#define J3041_1_MPS fixture_path("j3041_1.mps.gz")

/* Reference values (cross-validated) */
#define LP_FULL       0.12389597644749
#define LP_NO_S1_S2   0.02498365505193
#define LP_NO_S2      0.12389597644749   /* S1 back, S2 still absent (redundant) */
#define MIP_OPT       3.0
#define LP_TOL        1e-4
#define MIP_TOL       1e-2

#define S1_ROW  50   /* resR(1,15) */
#define S2_ROW  23   /* resR(0,23) */
...
static Cbc_Model *load_model(void)
{
  ...
  Cbc_setLogLevel(m, 0);
  int rc = Cbc_readMps(m, J3041_1_MPS);
  assert(rc == 0);
  return m;
}
...
static void test_model_dimensions(void)
{
  ...
  CHECK(Cbc_getNumRows(m) == 344, "expected 344 rows");
  CHECK(Cbc_getNumCols(m) == 801, "expected 801 cols");
  ...
  CHECK(nbin  == 736, "expected 736 binary vars");
  CHECK(nint  ==  33, "expected 33 general integer vars");
  CHECK(ncont ==  32, "expected 32 continuous vars");
...
static void test_pure_lp(void)
{
  ...
  CHECK(fabs(obj - LP_FULL) < LP_TOL,
        "LP obj must match reference value 0.12390");
...
static void test_lp_reoptimization(void)
{
  ...
  CHECK(fabs(obj0 - LP_NO_S1_S2) < LP_TOL,
        "step-0 LP obj must match reference 0.02498");
  ...
  CHECK(fabs(obj1 - LP_NO_S2) < LP_TOL,
        "step-1 LP obj must match reference 0.12390");
  ...
  CHECK(fabs(obj2 - LP_FULL) < LP_TOL,
        "step-2 LP obj must match full LP reference 0.12390");
...
static void test_mip_optimal(void)
{
  ...
  Cbc_setDblParam(m, DBL_PARAM_TIME_LIMIT, 180.0);
...
int main(void)
{
  printf("=== j3041_1 C-interface tests ===\n");
